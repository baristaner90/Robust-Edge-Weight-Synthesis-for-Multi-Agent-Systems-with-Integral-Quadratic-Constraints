function [Psi,Jpq] = jspec(Pi)
% function [Psi,M] = jspec(Pi)
%
% This function constructs a J-spectral factorization for Pi. In
% other words, it factorizes Pi as:
%   Pi = Psi'*M*Psi
...
%
% Note: Some technical conditions on Pi are required to obtain a
% J-spectral factorization.  A sufficient condition is for the upper
% left/lower right blocks of Pi to satisfy Pi_11>0 and Pi_22<0 on the 
% imaginary axis. This function assumes that the technical conditions
% required are satisfied by the multiplier Pi.

% 12/13/2013 PJS Update to IMPLICITDI for J-spectral factorizations
...
X0 = care(Apsi,Bpsi,Q,R,S);
Psi = ss(Apsi,Bpsi,Jpq*inv(W')*(Bpsi'*X0+S'),W);    
